...
function queryString() { if (!activeRange) return ''; return `?${new URLSearchParams(activeRange)}`; }
function localDayStart(value) { const dateValue = new Date(value); return new Date(dateValue.getFullYear(), dateValue.getMonth(), dateValue.getDate()); }
function rangeForPreset(preset) { if (preset === 'all') return null; const now = new Date(); const start = localDayStart(now); if (preset === 'today') return { from: start.toISOString(), to: now.toISOString() }; start.setDate(start.getDate() - (preset === '7d' ? 6 : 29)); return { from: start.toISOString(), to: now.toISOString() }; }
function customRange() { const from = document.getElementById('range-from').value; const to = document.getElementById('range-to').value; if (!from || !to) return null; const [fy, fm, fd] = from.split('-').map(Number); const [ty, tm, td] = to.split('-').map(Number); const start = new Date(fy, fm - 1, fd); const end = new Date(ty, tm - 1, td + 1); return start < end ? { from: start.toISOString(), to: end.toISOString() } : null; }
async function applyRange(range) { activeRange = range; selected = undefined; await refresh(); }
function setupTimeFilter() { const preset = document.getElementById('range-preset'); const custom = document.getElementById('custom-range'); preset.onchange = () => { custom.hidden = preset.value !== 'custom'; if (preset.value !== 'custom') void applyRange(rangeForPreset(preset.value)); }; document.getElementById('apply-range').onclick = () => { const range = customRange(); if (!range) { value('load-status', 'Escolha duas datas válidas para aplicar o filtro.'); return; } void applyRange(range); }; }
async function renderSessions() { const sessions = await read('/api/sessions' + queryString()); const list = document.getElementById('session-list'); sessions.forEach((session) => titles.set(session.sessionId, session.displayTitle)); if (!selected && sessions[0]) selected = sessions[0].sessionId; list.replaceChildren(...sessions.map(sessionButton)); }
